function [Params] = MakeSelectionMultiClick(Params)
...
case 'Character'
    if any(Params.Keyboard.State.InText)
        ...
        TargetText = Params.Keyboard.State.SelectableText{Params.Keyboard.State.InText}; % all text in target, chars are separated by ' '
        TargetTextSplit = strsplit(TargetText);
        TargetChar = TargetTextSplit(find(Cursor.ClickState));
        Params.Keyboard.State.SelectedCharacters = TargetChar;
